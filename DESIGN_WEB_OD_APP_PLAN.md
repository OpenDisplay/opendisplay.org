...
- **Classic↔module bridge:** `OpenDisplayBLE` is a top-level class in a classic script, not a `window` export. A short classic bridge script publishes exactly one global, `globalThis.odAppBle`; ES modules consume that named global via `ble-adapter.js` and nothing else. **The bridge creates a fresh `OpenDisplayBLE` instance for every connection** (see §3 — per-connection isolation) and rebinds `odAppBle` to it; the name is stable, the instance is not.
...
**M0 — Skeleton (2 d).** `httpdocs/app/` + `v1/` layout, nav link, capability gate with the site's standard messaging, classic-script bridge (`odAppBle`), schema-readiness (`adapter.ready()`), empty device list.
